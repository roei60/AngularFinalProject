...
//var keyWithMaximumValue;
var MaximumValue;
DestinationPopularity : any ="";
...
proto.update = function(key, v) {
  //console.log("Update with key : " + key + " and value is : " + v)
  var scratch = this.scratch
  var d = this.depth
  var w = this.width
  var tab = this.table
  ...
  for(var i=0; i<d; ++i) {
    ...
    ptr += w
    /*if(tab[ptr + (scratch[i] % w)] > MaximumValue){
        MaximumValue = tab[ptr + (scratch[i] % w)]
        keyWithMaximumValue = key;
    }*/
  }

    // check the maximum of the keys values
    var scratch = this.scratch
    var d = this.depth
    var w = this.width
    var tab = this.table
  ...
  //console.log("this.proto.query(key) : " + r);
  if(r > MaximumValue){
    //keyWithMaximumValue = key;
    this.DestinationPopularity = key;
    MaximumValue = r;
  }
}

proto.getMaximumKey = function(){
    //return keyWithMaximumValue;
    return this.DestinationPopularity;
}
